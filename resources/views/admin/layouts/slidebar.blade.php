<ul class="nav">
    ...
    @foreach (slidebar() as $menu)
        <li class="{{ isset($menu['childrens']) ? 'nav-item nav-dropdown' : 'nav-item' }}">
            <a class="{{ isset($menu['childrens']) ? 'nav-link nav-dropdown-toggle' : 'nav-link' }}"
                @if (isset($menu['link'])) href="{{ route($menu['link']) }}" @endif
                @if (isset($menu['url'])) href="{{ url()->action($menu['url']) }}" @endif>
                <i class="nav-icon {{ isset($menu['icon']) ? $menu['icon'] : 'icon-pencil' }}"></i>
                {{ $menu['name'] }}
                @if(isset($menu['badge']) && $menu['badge'])
                    <span class="badge badge-danger">{{ $menu['badge'] }}</span>
                @endisset
            </a>
            @if (isset($menu['childrens']))
                <ul class="nav-dropdown-items">
                    @foreach ($menu['childrens'] as $menuChild)
                        <li class="nav-item">
                            <a class="nav-link"
                                @if (isset($menuChild['link'])) href="{{ route($menuChild['link']) }}" @endif
                                @if (isset($menuChild['url'])) href="{{ url()->action($menuChild['url']) }}" @endif>
                                <i
                                    class="nav-icon
                        {{ isset($menuChild['icon']) ? $menuChild['icon'] : (isset($menu['icon']) ? $menu['icon'] : 'icon-pencil') }}
                    "></i>{{ $menuChild['name'] }}
                            </a>
                        </li>
                    @endforeach
